...
Java_com_example_yolo_1kotlin_1ncnn_NcnnDetector_detect(JNIEnv *env, jobject /* this */,
    ...
    // NCNN expects BGR format for normalization constants typically, but since we normalize
    // all channels the same way (divide by 255), the order doesn't matter here.
    // If mean subtraction was used, PIXEL_RGBA -> PIXEL_RGB -> PIXEL_BGR conversion might be needed first.
    ...
    { // Scope for extractor and mutex lock
        ...
        LOGI("Output tensor dimensions: w=%d, h=%d, c=%d, dims=%d", out.w, out.h, out.c, out.dims);
        ...
        // This part heavily depends on how your specific YOLOv11 model was exported to NCNN.
        // The current code assumes an output shape like [num_features, num_detections]
        // where num_features = 4 (bbox) + num_classes.
        // Example: out.w = 8400 (detections), out.h = 84 (4 bbox + 80 classes)
        ...
        // Access the output data. Assuming single channel output (out.c == 1)
        // If out.dims > 2 or out.c > 1, access needs adjustment (e.g., out.channel(c).row(h))
        ...
        for (int d = 0; d < num_detections; ++d) {
            ...
            // Pointer to the start of class scores for this detection 'd'
            // Scores start after the 4 bbox values.
            const float* class_scores_ptr = output_data + 4 * num_detections;
            for (int c = 0; c < NUM_CLASSES; ++c) {
                // Access score for class 'c' at detection 'd'
                // Data layout: [bbox_data][class0_data][class1_data]...
            ...
            if (max_class_prob >= CONFIDENCE_THRESHOLD) {
                // Extract bounding box coordinates (cx, cy, w, h) relative to padded input
                ...
                // Convert center coords (cx, cy) to top-left (x1, y1) relative to padded input
                float x1_padded = cx - w / 2.0f;
                float y1_padded = cy - h / 2.0f;

                // Convert coordinates from padded/scaled input back to original image coordinates
                ...
                x1_orig = std::max(0.0f, std::min((float)imageWidth - 1.0f, x1_orig)); // Use imageWidth-1 for max index
                y1_orig = std::max(0.0f, std::min((float)imageHeight - 1.0f, y1_orig)); // Use imageHeight-1 for max index
        ...
        // Sort objects by confidence score (descending) for NMS
    ...
    // *** ADDED LOGGING ***
    LOGI("Native detection found %d objects before formatting for Java.", final_count);
    // *** END ADDED LOGGING ***
    int result_size = 1 + final_count * 6; // 1 for count, 6 floats per detection (x, y, w, h, label, conf)
    ...
    resultData[0] = (float)final_count; // First element is the count
    ...
    LOGI("Native detection time: %lld ms", duration.count());
